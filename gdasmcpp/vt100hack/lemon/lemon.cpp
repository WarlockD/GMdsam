...
#define ISTERMINAL(X) ((ISALPHA((X)) && ISUPPER((X))) || ISDIGIT((X)))
...
struct rule {
	...
	bool doesReduce;      /* Reduce actions occur after optimization */
...
// Ugh I hate windows, it ahas  bunch of macros and it fucks this part up
enum  e_action {
	SHIFT,
	ACCEPT,
	REDUCE,
...
void Parse(struct lemon *gp)
{
	...
	for (cp = filebuf; (c = *cp) != 0; ) {
		...
		if (c == '\"') {                     /* String literals */
			...
			// added xX for hex numbers 
			while ((c = *cp) != 0 && (ISALNUM(c) || c == '_' || c == 'x' || c == 'X')) cp++;
			nextcp = cp;
		}
...
void ReportTable(
	...
	*/
	for (rp = lemp->rule; rp; rp = rp->next) rp->doesReduce = false;
	for (i = 0; i<lemp->nxstate; i++) {
		struct action *ap;
		for (ap = lemp->sorted[i]->ap; ap; ap = ap->next) {
			if (ap->type == REDUCE || ap->type == SHIFTREDUCE) {
				ap->x.rp->doesReduce = i ? true : false;
			}
		}
	}
