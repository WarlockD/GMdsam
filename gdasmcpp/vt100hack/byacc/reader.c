...
copy_text(void)
{
    ...
    if (!lflag)
	byacc_fprintf(f, line_format, lineno, input_file_name);
...
copy_union(void)
{
    ...
    if (!lflag)
	byacc_fprintf(text_file, line_format, lineno, input_file_name);
...
static bucket *
get_literal_made(unsigned char c)
{
	bucket *bp;
	...
	bp->class = TERM;
	if (bp->value == UNDEFINED) bp->value = UCH(c);
	return (bp);
}
...
insert_arg_rule(char *arg, char *tag)
{
	...
	byacc_fprintf(f, "case %d:\n", rule - 2);
	if (!lflag)
	    byacc_fprintf(f, line_format, line_number, input_file_name);
	byacc_fprintf(f, "%s;\n", code);
	byacc_fprintf(f, "break;\n");
	insert_empty_rule();
...
#endif
void
bad_char_range(int a_lineno)
{
	byacc_fprintf(stderr, "%s: w - line %d of \"%s\", bad char range\n", myname, a_lineno, input_file_name);
	exit(2);
}
static void
add_symbol(void)
...
copy_action(void)
{
  ...
  loop:
	...
	if (cptr[1] == '<')
	{
		...
		i = get_number();
		if (i == 0)
		    byacc_fprintf(f, "yystack.l_mark[%d].%s", -n, tag);
		else if (i > maxoffset)
		{
		    dollar_warning(d_lineno, i);
		    byacc_fprintf(f, "yystack.l_mark[%d].%s", i - maxoffset, tag);
		}
		else if (offsets)
		    byacc_fprintf(f, "yystack.l_mark[%d].%s", offsets[i], tag);
		FREE(d_line);
		goto loop;
	    }
		...
		++cptr;
		i = -get_number() - n;
		byacc_fprintf(f, "yystack.l_mark[%d].%s", i, tag);
		FREE(d_line);
		goto loop;
	    }
#if defined(YYBTYACC)
		...
		byacc_fprintf(f, "yystack.l_mark[%d].%s",
	...
	else if (isdigit(UCH(cptr[1])))
	{
	    ...
	    {
		if (i == 0)
		    byacc_fprintf(f, "yystack.l_mark[%d]", -n);
		else if (i > maxoffset)
		{
		    dollar_warning(lineno, i);
		    byacc_fprintf(f, "yystack.l_mark[%d]", i - maxoffset);
		}
		else if (offsets)
		    byacc_fprintf(f, "yystack.l_mark[%d]", offsets[i]);
	    }
	    goto loop;
	}
	else if (cptr[1] == '-')
	{
	    ...
	    if (havetags)
		unknown_rhs(-i);
	    byacc_fprintf(f, "yystack.l_mark[%d]", -i - n);
	    goto loop;
	}
#if defined(YYBTYACC)
	else if (isalpha(cptr[1]) || cptr[1] == '_')
	{
	    ...
	    byacc_fprintf(f, "yystack.l_mark[%d]", i - plhs[nrules]->args + 1 - n);
	    if (tag)
		byacc_fprintf(f, ".%s", tag);
	    else if (havetags)
	...
	else if (isdigit(UCH(cptr[1])))
	{
	    ...
	    i = get_number();
	    if (i == 0)
		byacc_fprintf(f, "yystack.p_mark[%d]", -n);
	    else if (i > maxoffset)
	    {
		at_warning(lineno, i);
		byacc_fprintf(f, "yystack.p_mark[%d]", i - maxoffset);
	    }
	    else if (offsets)
		byacc_fprintf(f, "yystack.p_mark[%d]", offsets[i]);
	    goto loop;
	}
	else if (cptr[1] == '-')
	{
	    cptr += 2;
	    i = get_number();
	    byacc_fprintf(f, "yystack.p_mark[%d]", -i - n);
	    goto loop;
	}
    }
#endif
...
read_grammar(void)
{
	...
	for (;;)
	{
		...
		if (isalpha(c)
			...
			add_symbol();
			if (in_range) {
				bp_end = pitem[nitems - 1];
				end_rule();
				for (int i = bp_start->value+1; i < bp_end->value; i++) {
					start_rule(plhs[nrules - 1], 0);
					bucket *bp3 = get_literal_made(i);
...
print_grammar(void)
{
	...
	if (rlhs[i] != rlhs[i - 1])
	{
	    if (i != 2)
		byacc_fprintf(f, "\n");
	    byacc_fprintf(f, "%4d  %s :", i - 2, symbol_name[rlhs[i]]);
	    spacing = strlen(symbol_name[rlhs[i]]) + 1;
	}
	else
	{
	    byacc_fprintf(f, "%4d  ", i - 2);
	    j = spacing;
	    while (j-- != 0)
		putc(' ', f);
	    putc('|', f);
	...
	while (ritem[k] >= 0)
	{
	    byacc_fprintf(f, " %s", symbol_name[ritem[k]]);
	    ++k;
	}
	++k;
	putc('\n', f);
