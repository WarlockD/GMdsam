...
verbose(void)
{
    ...
    byacc_fprintf(verbose_file, "\n\n%d terminals, %d nonterminals\n", ntokens,
	    nvars);
    byacc_fprintf(verbose_file, "%d grammar rules, %d states\n", nrules - 2, nstates);
#if defined(YYBTYACC)
	...
	for (i = 0; i < ntokens; ++i)
	{
	    byacc_fprintf(verbose_file, " %5d  %5d  %s\n",
...
log_unused(void)
{
	...
	if (!rules_used[i])
	{
	    byacc_fprintf(verbose_file, "\t%s :", symbol_name[rlhs[i]]);
	    for (p = ritem + rrhs[i]; *p >= 0; ++p)
		byacc_fprintf(verbose_file, " %s", symbol_name[*p]);
	    byacc_fprintf(verbose_file, "  (%d)\n", i - 2);
	}
    }
}
...
log_conflicts(void)
{
	...
	{
	    byacc_fprintf(verbose_file, "State %d contains ", i);
	    if (SRconflicts[i] > 0)
		byacc_fprintf(verbose_file, "%d shift/reduce conflict%s",
			SRconflicts[i],
			PLURAL(SRconflicts[i]));
	    if (SRconflicts[i] && RRconflicts[i])
		byacc_fprintf(verbose_file, ", ");
	    if (RRconflicts[i] > 0)
		byacc_fprintf(verbose_file, "%d reduce/reduce conflict%s",
			RRconflicts[i],
...
print_state(int state)
{
    ...
    byacc_fprintf(verbose_file, "state %d\n", state);
    print_core(state);
    print_nulls(state);
    print_actions(state);
}
...
print_conflicts(int state)
{
	...
	else if (p->suppressed == 1)
	{
	    ...
	    {
		byacc_fprintf(verbose_file, "%d: shift/reduce conflict \
...
(shift %d, reduce %d) on %s\n", state, number, p->number - 2,
		...
		{
		    byacc_fprintf(verbose_file, "%d: reduce/reduce conflict \
...
print_core(int state)
{
	...
	while (*sp >= 0)
	    ++sp;
	rule = -(*sp);
	byacc_fprintf(verbose_file, "\t%s : ", symbol_name[rlhs[rule]]);

	for (sp = ritem + rrhs[rule]; sp < sp1; sp++)
	    byacc_fprintf(verbose_file, "%s ", symbol_name[*sp]);
...
print_nulls(int state)
{
    ...
    {
	j = null_rules[i];
	byacc_fprintf(verbose_file, "\t%s : .  (%d)\n", symbol_name[rlhs[j]],
		j - 2);
    }
    byacc_fprintf(verbose_file, "\n");
}
...
print_actions(int stateno)
{
	...
	byacc_fprintf(verbose_file, "\t$end  accept\n");
...
print_shifts(action *p)
{
	...
	for (; p; p = p->next)
	{
		...
		byacc_fprintf(verbose_file, "\t%s  shift %d\n",
			symbol_name[p->symbol], p->number);
#if defined(YYBTYACC)
	    if (backtrack && p->action_code == SHIFT && p->suppressed == 1)
		byacc_fprintf(verbose_file, "\t%s  [trial] shift %d\n",
			symbol_name[p->symbol], p->number);
#endif
	}
    }
...
print_reductions(action *p, int defred2)
{
    ...
    if (anyreds == 0)
	byacc_fprintf(verbose_file, "\t.  error\n");
    else
    {
	for (; p; p = p->next)
	{
		...
		k = p->number - 2;
		if (p->suppressed == 0)
		    byacc_fprintf(verbose_file, "\t%s  reduce %d\n",
			    symbol_name[p->symbol], k);
#if defined(YYBTYACC)
		if (backtrack && p->suppressed == 1)
		    byacc_fprintf(verbose_file, "\t%s  [trial] reduce %d\n",
			    symbol_name[p->symbol], k);
#endif
	    }
	}
	...
	if (defred2 > 0)
	    byacc_fprintf(verbose_file, "\t.  reduce %d\n", defred2 - 2);
    }
}
...
print_gotos(int stateno)
{
	...
	if (ISVAR(as))
	    byacc_fprintf(verbose_file, "\t%s  goto %d\n", symbol_name[as], k);
    }
}
