...
unused_rules(void)
{
    ...
    {
	if (nunused == 1)
	    byacc_fprintf(stderr, "%s: 1 rule never reduced\n", myname);
	else
	    byacc_fprintf(stderr, "%s: %d rules never reduced\n", myname, nunused);
    }
}
...
{
    byacc_fprintf(stderr, "%s: ", myname);
    if (SRtotal == 1)
	byacc_fprintf(stderr, "1 shift/reduce conflict");
    else if (SRtotal > 1)
	byacc_fprintf(stderr, "%d shift/reduce conflicts", SRtotal);

    if (SRtotal && RRtotal)
	byacc_fprintf(stderr, ", ");

    if (RRtotal == 1)
	byacc_fprintf(stderr, "1 reduce/reduce conflict");
    else if (RRtotal > 1)
	byacc_fprintf(stderr, "%d reduce/reduce conflicts", RRtotal);
    ...
    {
	byacc_fprintf(stderr, "%s: ", myname);
	byacc_fprintf(stderr, "expected %d shift/reduce conflict%s.\n",
    ...
    {
	byacc_fprintf(stderr, "%s: ", myname);
	byacc_fprintf(stderr, "expected %d reduce/reduce conflict%s.\n",
