...
puts_param_types(FILE * fp, param *list, int more)
{
	...
	for (p = list; p; p = p->next)
	{
		...
		byacc_fprintf(fp, "%s%s%s%s%s", p->type,
...
write_code_lineno(FILE * fp)
{
    ...
    {
	++outline;
	byacc_fprintf(fp, line_format, outline + 1, code_file_name);
    }
}
...
write_input_lineno(void)
{
    ...
    {
	++outline;
	byacc_fprintf(code_file, line_format, lineno, input_file_name);
    }
}
...
define_prefixed(FILE * fp, const char *name)
{
    ...
    if (bump_line)
	++outline;
    byacc_fprintf(fp, "#ifndef %s\n", name);

    if (bump_line)
	++outline;
    byacc_fprintf(fp, "#define %-10s %s%s\n", name, symbol_prefix, name + 2);

    if (bump_line)
	++outline;
    byacc_fprintf(fp, "#endif /* %s */\n", name);
}
...
output_prefix(FILE * fp)
{
    ...
    if (CountLine(fp))
	++outline;
    byacc_fprintf(fp, "#define YYPREFIX \"%s\"\n", symbol_prefix);
}
...
{
    byacc_fprintf(output_file, "%5d,", value);
}
...
output_YYINT_typedef(FILE * fp)
{
    ...
    if (CountLine(fp))
	++outline;
    byacc_fprintf(fp, "typedef %s YYINT;\n", CONCAT1("", YYINT));
}
...
output_table(void)
{
    ...
    if (high >= MAXYYINT)
    {
	byacc_fprintf(stderr, "YYTABLESIZE: %ld\n", high);
	byacc_fprintf(stderr, "Table is longer than %d elements.\n", MAXYYINT);
	done(1);
    }

    ++outline;
    byacc_fprintf(code_file, "#define YYTABLESIZE %ld\n", high);
...
{
    byacc_fprintf(defines_file, "#ifndef _%s_defines_h_\n", symbol_prefix);
    byacc_fprintf(defines_file, "#define _%s_defines_h_\n\n", symbol_prefix);
}

static void
end_defines_file(void)
{
    byacc_fprintf(defines_file, "\n#endif /* _%s_defines_h_ */\n", symbol_prefix);
}
#else
...
output_defines(FILE * fp)
{
	...
	byacc_fprintf(fp, "#define YYERRCODE %d\n", symbol_value[1]);
	...
	if (unionized)
	{
	    ...
	    byacc_fprintf(fp, "extern YYSTYPE %slval;\n", symbol_prefix);
	}
    }
}
...
output_debug(void)
{
    ...
    ++outline;
    byacc_fprintf(code_file, "#define YYFINAL %d\n", final_state);

    putl_code(code_file, "#ifndef YYDEBUG\n");
    ++outline;
    byacc_fprintf(code_file, "#define YYDEBUG %d\n", tflag);
    putl_code(code_file, "#endif\n");

    if (rflag)
    {
	byacc_fprintf(output_file, "#ifndef YYDEBUG\n");
	byacc_fprintf(output_file, "#define YYDEBUG %d\n", tflag);
	byacc_fprintf(output_file, "#endif\n");
    }
    ...
    ++outline;
    byacc_fprintf(code_file, "#define YYMAXTOKEN %d\n", maxtok);

    ++outline;
    byacc_fprintf(code_file, "#define YYUNDFTOKEN %d\n", max + 1);

    ++outline;
    byacc_fprintf(code_file, "#define YYTRANSLATE(a) ((a) > YYMAXTOKEN ? "
	    "YYUNDFTOKEN : (a))\n");
    ...
    for (i = 2; i < nrules; ++i)
    {
	byacc_fprintf(output_file, "\"%s :", symbol_name[rlhs[i]]);
	for (j = rrhs[i]; ritem[j] > 0; ++j)
	{
		...
		while (*++s != '"')
		{
			...
			if (s[1] == '\\')
			    ...
			    byacc_fprintf(output_file, "\\\\%c", s[1]);
			++s;
		    }
		    else
			putc(*s, output_file);
		...
		else if (s[1] == '\\')
		{
			...
			byacc_fprintf(output_file, " '\\\\%c", s[2]);
		    s += 2;
		    while (*++s != '\'')
			putc(*s, output_file);
		    putc('\'', output_file);
...
output_pure_parser(FILE * fp)
{
    ...
    if (fp == code_file)
	++outline;
    byacc_fprintf(fp, "#define YYPURE %d\n", pure_parser);
    putc_code(fp, '\n');
}
...
output_yydestruct_impl(void)
{
	...
	{
	    ++outline;
	    byacc_fprintf(code_file, "\tcase %d:\n", symbol_pval[i]);
...
output(void)
{
	...
	write_code_lineno(code_file);
	++outline;
	byacc_fprintf(code_file, "#include \"%s\"\n", externs_file_name);
	fp = externs_file;
    }
    else
	fp = code_file;
