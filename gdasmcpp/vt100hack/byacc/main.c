...
usage(void)
{
    ...
    fflush(stdout);
    byacc_fprintf(stderr, "Usage: %s [options] filename\n", myname);
    for (n = 0; n < sizeof(msg) / sizeof(msg[0]); ++n)
	byacc_fprintf(stderr, "%s\n", msg[n]);
...
open_files(void)
{
	...
	byacc_fprintf(graph_file, "digraph %s {\n", file_prefix);
	byacc_fprintf(graph_file, "\tedge [fontsize=10];\n");
	byacc_fprintf(graph_file, "\tnode [shape=box,fontsize=10];\n");
	byacc_fprintf(graph_file, "\torientation=landscape;\n");
	byacc_fprintf(graph_file, "\trankdir=LR;\n");
	byacc_fprintf(graph_file, "\t/*\n");
	byacc_fprintf(graph_file, "\tmargin=0.2;\n");
	byacc_fprintf(graph_file, "\tpage=\"8.27,11.69\"; // for A4 printing\n");
	byacc_fprintf(graph_file, "\tratio=auto;\n");
	byacc_fprintf(graph_file, "\t*/\n");
    }
